##### Edit 2026, added a txt file for metrics that are useful to monitor for security and reliability. Systemcollector is a mandatory collector I believe so you should not exclude it. Routingentry collector takes more time to process than the other collectors because it must fetch new data about all routes, thats why its useful to put the field routing-database-hours to a higher number if youre more concerned with exporter performance than a 100% reliable metrics. According to Ribbon the load on the SBCs is minimal with this exporter.
